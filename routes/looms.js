...
router.get('/', async (req, res) => {
    try {
        ...
        const loomsRef = db.collection('looms');
        const snapshot = await loomsRef.orderBy('createdAt', 'desc').get();
        
        const looms = [];
        for (const doc of snapshot.docs) {
            const loomData = doc.data();
            
            // For busy looms, get the associated warp and order information
            let warpData = null;
            let orderData = null;
            let fabricCutStats = null;
            
            if (loomData.status === 'busy') {
                // Find the active warp that's using this loom
                ...
                if (!warpsSnapshot.empty) {
                    ...
                    // Get fabric cut statistics for this warp
                    const fabricCutsSnapshot = await db.collection('fabricCuts')
                        .where('warpId', '==', warpData.id)
                        .get();
                    
                    let totalFabricCuts = 0;
        ...
        console.error('Error in looms route:', err);
